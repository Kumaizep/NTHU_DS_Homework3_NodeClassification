...
from dgl.nn.pytorch import GraphConv, SAGEConv, GATv2Conv, DenseGraphConv
...
class GCN(nn.Module):
    ...
    def __init__(self, in_size, hid_size, out_size):
        ...
        self.layers.append(
            ...
            SAGEConv(in_size, hid_size, 'mean', activation=F.relu)
            # DenseGraphConv(in_size, hid_size, activation=F.relu)
            )
        self.layers.append(
            # GraphConv(hid_size, out_size)
            SAGEConv(hid_size, out_size, 'mean')
            # DenseGraphConv(hid_size, out_size)
            )
